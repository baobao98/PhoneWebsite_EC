...
Invoices.post('/thongke', async (req, res) => {

    let nosent;
    let nodelivery;
    await Invoice.count({ state: 'Sent' }, (err, count) => {
        if (err) console.log(err);
        nosent = count;
    })
    await Invoice.count({ state: 'Delivering' }, (err, count) => {
        if (err) console.log(err);
        nodelivery = count;
    })
    let { noInvoice, total } = await getTotalnInvoice();

    res.json({ "nosent": nosent, "nodelivery": nodelivery, "total": total, "noInvoice": noInvoice });
    // console.log(total,noInvoice);
})
async function getTotalnInvoice() {
    let now = new Date();
    let no = 0;
    let total = 0;
    await Invoice.find({}, (err, i) => {
        if (err) console.log(err);
        i.forEach(x => {
            // console.log(x.dateOrdered.getMonth());
            if (x.dateOrdered.getMonth() == now.getMonth()) {
                no++;
            }
            x.products.forEach(item => {
                total += item.quantity + item.product.promotion;
            })
        })
    });
    return {
...
//     mailService.send();
// })
Invoices.get('/tracking/:invoice_id', (req, res) => {
    Invoice.find({ code: req.params.invoice_id }, function (err, inv) {
...
Invoices.route('/:invoice_id')
    ...
    .get((req, res) => {
        Invoice.findById({ _id: req.params.invoice_id }, function (err, inv) {
            if (err) return res.send(err);
